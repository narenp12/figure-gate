...
without writing, which is how a change is measured against this example:
    ...
    import demo
    fig, passed = demo.build(out=None)
"""
...
with plt.style.context(str(STYLE)):
    ...
    # hardcoded white -- a sheet that changes `axes.facecolor` would otherwise
    # draw every label with a bright outline around it.
...
def styled(fn):
    """Run the builder with the sheet in effect, and only then.

    The sheet has to be live while the figure is constructed and still live
    through `cf.report`, because `check_style_sheet` compares the rcParams that
    are in effect against the sheet on disk. Both happen inside `build`.
    """
    @functools.wraps(fn)
    def run(*args, **kwargs):
        with plt.style.context(str(STYLE)):
...
def build(out=HERE):
    ...
    path = Path(out) / "demo.png"
    fig.savefig(path, metadata=cf.alt_metadata(fig, path))
    plt.close(fig)
    print(f"wrote {path}")
    return fig, passed
